...
!  1/31/21. Version 2.8.3, Include 3 more taylor routines (from the LIDORT code)
!    -- These are needed for the Green's function solutions
...
!  Arguments
   ...
   integer  , intent(in)  :: order
   double precision, intent(in)  :: eps, y, delta, fac1, fac2, sm
   double precision, intent(out) :: mult

!  Local

   integer    :: mterms, m, j
   double precision  :: d(0:max_Taylor_terms), ac(0:max_Taylor_terms), cc(0:max_Taylor_terms), COF_0
   double precision  :: Term_1(0:max_Taylor_terms),Term_2
   double precision  :: Y1, power, power2
...
!  Arguments
   ...
   integer  , intent(in)  :: order
   double precision, intent(in)  :: eps, y, delta, deldot, lamdot, kdot, wudel, sm
   double precision, intent(out) :: L_mult

!  Local

   integer    :: mterms1, mterms2, m, j
   double precision  :: d(0:max_Taylor_terms), ac(0:max_Taylor_terms), bc(0:max_Taylor_terms)
   double precision  :: cc(0:max_Taylor_terms), dc(0:max_Taylor_terms), COF_0
   double precision  :: Term_1a(0:max_Taylor_terms), Term_1b(0:max_Taylor_terms), Term_3(0:max_Taylor_terms), Term_2
   double precision  :: CONS1, CONS2, Y1, Nterm, power
...
!  Develop ( 1 - x/s )^-2  for small x, need Order+2 terms
   ...
   do m = 1, mterms2
      ac(m) = y1 * ac(m-1)
      bc(m) = y1 * bc(m-1) * dble(m+1) / dble(m)
   enddo
...
!  Arguments
   ...
   integer  , intent(in)  :: order
   double precision, intent(in)  :: eps, y, delta, deldot, lamdot, kdot, wdel, udel, sm, lam
   double precision, intent(out) :: L_mult
!  Local
   ...
   double precision  :: cc(0:max_Taylor_terms), dc(0:max_Taylor_terms), COF_0
   double precision  :: Term_1a(0:max_Taylor_terms), Term_1b(0:max_Taylor_terms), Term_3(0:max_Taylor_terms), Term_2
   double precision  :: CONS1, CONS2, CONS3, Y1, Nterm, power
