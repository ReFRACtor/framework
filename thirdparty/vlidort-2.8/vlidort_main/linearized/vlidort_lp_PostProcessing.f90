...
!           ** Use post-processing mask for Observational/Lattice-Doublet distinction
      ...
      MODULE vlidort_lp_PostProcessing_m
...
!
      ...
      SUBROUTINE LP_WHOLELAYER_STERM_UP ( &
             ...
             LAYER, IBEAM, NV, NV_PARAMETERS, NSTOKES, N_PPSTREAMS, PPSTREAM_MASK, & ! Input numbers
...
!    -- Introduce control flag DO_CLASSICAL_SOLUTION, and Taylor-series order
...
!  1/31/21. Version 2.8.3. New Help variables for the Green's function solution
      ...
      DOUBLE PRECISION :: L_FIRST, L_DELTA, L_KEG, L_LAM, L_GAMMA, L_WDEL
...
!mick fix 1/5/2021 - reapplied "N.EQ.NV" IF condition
...
!  Nothing further to do if Varying layer is below NV
!    -- 1/31/21. Version 2.8.3. This condition added
...
!     -- Special case when N = NV, needs the LP_UPAR_UP_2 linearization
        ...
        IF ( N.EQ.NV ) THEN
          DO LUM = 1, N_PPSTREAMS
            ...
            DO O1 = 1, NSTOKES
              DO Q = 1, NV_PARAMETERS
                SPAR = LP_UPAR_UP_2(UM,O1,N,NV,Q) *   EMULT_UP(LUM,N,IB) + &
                          UPAR_UP_2(UM,O1,N)      * LP_EMULT_UP(LUM,N,NV,IB,Q)
...
!     ==> Distinguish between cases N = NV and N > NV

      IF ( .not. DO_CLASSICAL_SOLUTION ) THEN

!  This section for N = NV
!  -----------------------
...
!                    for consistency with WHOLELAYER_STERM_UP
          ...
          DO Q = 1, NV_PARAMETERS
            L_WDEL = LP_T_DELT_MUBAR(N,NV,IB,Q)
            L_ITRANS(Q)     = LP_INITIAL_TRANS(N,NV,IB,Q) * ITRANS   ! Zero here
...
!                MULTDN = PMULT_UD(K,UM,N) / ATERM_SAVE(K,N) ; IGAM = ONE / GAMMA_M(K,N)
                ...
                DO Q = 1, NV_PARAMETERS
                  L_GAMMA  = LP_AVERAGE_SECANT(N,NV,IB,Q) - L_KEIGEN(K,N,Q)
                  L_FIRST  = ITRANS * L_HMULT_2(K,UM,N,Q) + L_ITRANS(Q) * HMULT_2(K,UM,N)
                  L_MULTDN(Q) = ( L_FIRST - LP_EMULT_UP(LUM,N,NV,IB,Q) - L_GAMMA * MULTDN ) * IGAM
                ENDDO
              endif
...
!mick mod 1/5/2021 - removed neg. sign from L_FIRST for consistency with WHOLELAYER_STERM_UP
              ...
              DO Q = 1, NV_PARAMETERS
                ...
                L_MULTUP(Q) = ( L_FIRST + LP_EMULT_UP(LUM,N,NV,IB,Q) - L_GAMMA * MULTUP ) * IGAM
              ENDDO
...
!                    for consistency with WHOLELAYER_STERM_UP
          ...
          DO Q = 1, NV_PARAMETERS
            L_WDEL = LP_T_DELT_MUBAR(N,NV,IB,Q)
            L_ITRANS(Q)     = LP_INITIAL_TRANS(N,NV,IB,Q) * ITRANS        ! Non-zero
            L_ITRANSWDEL(Q) = - (ITRANS * L_WDEL + WDEL * L_ITRANS(Q))
          ENDDO
...
!                MULTDN = PMULT_UD(K,UM,N) / ATERM_SAVE(K,N) ; IGAM = ONE / GAMMA_M(K,N)
                ...
                MULTDN = PMULT_UD(K,UM,N) ; IGAM = ONE / GAMMA_M(K,N)
                DO Q = 1, NV_PARAMETERS
                  L_GAMMA  = LP_AVERAGE_SECANT(N,NV,IB,Q)
                  L_FIRST  = L_ITRANS(Q) * HMULT_2(K,UM,N)
                  L_MULTDN(Q) = ( L_FIRST - LP_EMULT_UP(LUM,N,NV,IB,Q) - L_GAMMA * MULTDN ) * IGAM
                ENDDO
              endif
...
!mick mod 1/5/2021 - removed neg. sign from L_FIRST for consistency with WHOLELAYER_STERM_UP
              ...
              DO Q = 1, NV_PARAMETERS
                L_GAMMA  = LP_AVERAGE_SECANT(N,NV,IB,Q)
                L_FIRST  = L_ITRANSWDEL(Q) * HMULT_1(K,UM,N)
                L_MULTUP(Q) = ( L_FIRST + LP_EMULT_UP(LUM,N,NV,IB,Q) - L_GAMMA * MULTUP ) * IGAM
              ENDDO
...
!  Complete the multipliers
              ...
              DO Q = 1, NV_PARAMETERS
                L_PMULT_DN(Q) = ATERM_SAVE(K,N) * L_MULTDN(Q) 
                L_PMULT_UP(Q) = BTERM_SAVE(K,N) * L_MULTUP(Q) 
              ENDDO
...
!  Add the particular solution contributions
               ...
               DO Q = 1, NV_PARAMETERS
                  DO O1 = 1, NSTOKES
                     H1 = UHOM_UPDN(UM,O1,K,N) * L_PMULT_DN(Q)
                     H2 = UHOM_UPUP(UM,O1,K,N) * L_PMULT_UP(Q)
                     SPARG(O1,Q) = SPARG(O1,Q) + H1 + H2
                  ENDDO
               ENDDO
...
!  1/31/21. Version 2.8.3. Applies to Green's function or classical solution
!    -- Use of post-processing mask
!    -- Again, distinguish case N = NV against N > NV
      ...
      IF ( .NOT. DO_MSMODE_VLIDORT ) THEN
        IF ( N.EQ.NV ) THEN
          DO LUM = 1, N_PPSTREAMS
            ...
            DO O1 = 1, NSTOKES
              DO Q = 1, NV_PARAMETERS
                SPAR = L_UPAR_UP_1(UM,O1,N,Q) *   EMULT_UP(LUM,N,IB) + &
                         UPAR_UP_1(UM,O1,N)   * LP_EMULT_UP(LUM,N,NV,IB,Q)
                L_LAYERSOURCE(UM,O1,Q) = L_LAYERSOURCE(UM,O1,Q) + SPAR
              ENDDO
            ENDDO
          ENDDO
        ELSE IF ( N.gt.NV ) THEN
          DO LUM = 1, N_PPSTREAMS
            ...
            DO O1 = 1, NSTOKES
              DO Q = 1, NV_PARAMETERS
                SPAR = UPAR_UP_1(UM,O1,N)   * LP_EMULT_UP(LUM,N,NV,IB,Q)
                L_LAYERSOURCE(UM,O1,Q) = L_LAYERSOURCE(UM,O1,Q) + SPAR
              ENDDO
            ENDDO
          ENDDO
...
!  1/31/21. Version 2.8.3. New Help variables for the Green's function solution
      ...
      DOUBLE PRECISION :: L_FIRST, L_DELTA, L_KEG, L_LAM, L_GAMMA, L_WDEL, LAM, UDEL
      DOUBLE PRECISION :: L_MULTDN(MAX_ATMOSWFS)  , L_MULTUP(MAX_ATMOSWFS)
      DOUBLE PRECISION :: L_ITRANS(MAX_ATMOSWFS)  , L_ITRANSWDEL(MAX_ATMOSWFS)
      DOUBLE PRECISION :: L_PMULT_UP(MAX_ATMOSWFS), L_PMULT_DN(MAX_ATMOSWFS)
      DOUBLE PRECISION :: SPARG(MAXSTOKES,MAX_ATMOSWFS)
...
!     -- Special case when N = NV, needs the LP_UPAR_UP_2 linearization
        ...
        IF ( N.EQ.NV ) THEN
          DO LUM = 1, N_PPSTREAMS
            ...
            DO O1 = 1, NSTOKES
              DO Q = 1, NV_PARAMETERS
                SPAR = LP_UPAR_DN_2(UM,O1,N,NV,Q) *   EMULT_DN(LUM,N,IB) + &
                          UPAR_DN_2(UM,O1,N)      * LP_EMULT_DN(LUM,N,NV,IB,Q)
                L_LAYERSOURCE(UM,O1,Q) = L_LAYERSOURCE(UM,O1,Q) + SPAR
              ENDDO
            ENDDO
          ENDDO
...
!                MULTDN = PMULT_DD(K,UM,N) / ATERM_SAVE(K,N) ; IGAM = ONE / GAMMA_M(K,N)
                ...
                DO Q = 1, NV_PARAMETERS
                  ...
                  L_MULTDN(Q) = ( L_FIRST - LP_EMULT_DN(LUM,NV,N,IB,Q) - L_GAMMA * MULTDN ) * IGAM
                ENDDO
              endif
...
!              MULTUP = PMULT_DU(K,UM,N) / BTERM_SAVE(K,N) ; IGAM = ONE / GAMMA_P(K,N)
              ...
              DO Q = 1, NV_PARAMETERS
                ...
                L_MULTUP(Q) = ( L_FIRST + LP_EMULT_DN(LUM,N,NV,IB,Q) - L_GAMMA * MULTUP ) * IGAM
              ENDDO
...
!                MULTDN = PMULT_DD(K,UM,N) / ATERM_SAVE(K,N) ; IGAM = ONE / GAMMA_M(K,N)
                ...
                DO Q = 1, NV_PARAMETERS
                  L_GAMMA  = LP_AVERAGE_SECANT(N,NV,IB,Q) 
                  L_FIRST  = L_ITRANS(Q) * HMULT_1(K,UM,N)
                  L_MULTDN(Q) = ( L_FIRST - LP_EMULT_DN(LUM,N,NV,IB,Q) - L_GAMMA * MULTDN ) * IGAM
                ENDDO
              endif
...
!              MULTUP = PMULT_DU(K,UM,N) / BTERM_SAVE(K,N) ; IGAM = ONE / GAMMA_P(K,N)
              ...
              DO Q = 1, NV_PARAMETERS
                L_GAMMA  = LP_AVERAGE_SECANT(N,NV,IB,Q)
                L_FIRST  = L_ITRANSWDEL(Q) * HMULT_2(K,UM,N)
                L_MULTUP(Q) = ( L_FIRST + LP_EMULT_DN(LUM,N,NV,IB,Q) - L_GAMMA * MULTUP ) * IGAM
              ENDDO
...
!  Add the particular solution contributions
              ...
              DO Q = 1, NV_PARAMETERS
                DO O1 = 1, NSTOKES
                  H1 = UHOM_DNDN(UM,O1,K,N) * L_PMULT_DN(Q)
                  H2 = UHOM_DNUP(UM,O1,K,N) * L_PMULT_UP(Q)
                  SPARG(O1,Q) = SPARG(O1,Q) + H1 + H2
                ENDDO
              ENDDO
      ...
      IF ( .NOT. DO_MSMODE_VLIDORT ) THEN
            ...
            DO O1 = 1, NSTOKES
              DO Q = 1, NV_PARAMETERS
                SPAR = L_UPAR_DN_1(UM,O1,N,Q) *   EMULT_DN(LUM,N,IB) + &
                         UPAR_DN_1(UM,O1,N)   * LP_EMULT_DN(LUM,N,NV,IB,Q)
                L_LAYERSOURCE(UM,O1,Q) = L_LAYERSOURCE(UM,O1,Q) + SPAR
              ENDDO
            ENDDO
          ENDDO
        ELSE IF ( N.gt.NV ) THEN
          DO LUM = 1, N_PPSTREAMS
            ...
            DO O1 = 1, NSTOKES
              DO Q = 1, NV_PARAMETERS
                SPAR = UPAR_DN_1(UM,O1,N)   * LP_EMULT_DN(LUM,N,NV,IB,Q)
                L_LAYERSOURCE(UM,O1,Q) = L_LAYERSOURCE(UM,O1,Q) + SPAR
              ENDDO
            ENDDO
          ENDDO
...
!
      ...
      SUBROUTINE LP_PARTLAYER_STERM_UP ( &
           DO_SOLAR_SOURCES, DO_INCLUDE_THERMEMISS, DO_THERMAL_TRANSONLY,                & ! Input flags
           DO_CLASSICAL_SOLUTION, DO_MSMODE_VLIDORT, SOURCETERM_FLAG, FOURIER, IB,       & ! Input flags/numbers
...
!  ---------------
      ...
      INTEGER ::          UM, O1, Q, M
      INTEGER ::          K, KO1, K0, K1, K2, LUM
      DOUBLE PRECISION :: SHOM_R, SHOM_CR, SP, H1, H2, TM, TUDA, TUUB
      DOUBLE PRECISION :: LUXR, MUXR, NUXR, PUXR, LLUXR, MLUXR
      DOUBLE PRECISION :: LUXR1, MUXR1, NUXR1, PUXR1, LLUXR1, MLUXR1
      DOUBLE PRECISION :: LUXR2, MUXR2, NUXR2, PUXR2, LLUXR2, MLUXR2
...
!  1/31/21. Version 2.8.3. New Help variables for the Green's function solution
      ...
      DOUBLE PRECISION :: ITRANS, WDEL, ITRANSWDEL, MULTDN, MULTUP, EPS, DELTA, PARTA, YFAC
      DOUBLE PRECISION :: IGAM, LAM, UXUP, SM, FAC1, FAC2, MULT1, MULT2
      DOUBLE PRECISION :: L_FIRST, L_PARTA, L_DELTA, L_KEG, L_LAM, L_GAMMA, L_WDEL, L_MULT1, L_MULT2, L_MULT, L_UXUP
      DOUBLE PRECISION :: L_MULTDN(MAX_ATMOSWFS)  , L_MULTUP(MAX_ATMOSWFS)
      DOUBLE PRECISION :: L_ITRANS(MAX_ATMOSWFS)  , L_ITRANSWDEL(MAX_ATMOSWFS)
      DOUBLE PRECISION :: L_PMULT_UP(MAX_ATMOSWFS), L_PMULT_DN(MAX_ATMOSWFS)
      DOUBLE PRECISION :: SPARG(MAXSTOKES,MAX_ATMOSWFS)
...
!     -- Special case when N = NV, needs the LP_UPAR_UP_2 linearization
        ...
        IF ( N.EQ.NV ) THEN
          DO LUM = 1, N_PPSTREAMS
            ...
            DO O1 = 1, NSTOKES
              DO Q = 1, NV_PARAMETERS
                SP = LP_UPAR_UP_2(UM,O1,N,NV,Q) *   UT_EMULT_UP(LUM,UT,IB) + &
                        UPAR_UP_2(UM,O1,N)      * LP_UT_EMULT_UP(LUM,UT,NV,IB,Q)
...
!                    for consistency with PARTLAYER_STERM_UP
          ...
          DO Q = 1, NV_PARAMETERS
            L_WDEL = LP_T_DELT_MUBAR(N,NV,IB,Q)
            L_ITRANS(Q)     = LP_INITIAL_TRANS(N,NV,IB,Q) * ITRANS
            L_ITRANSWDEL(Q) = - (ITRANS * L_WDEL + WDEL * L_ITRANS(Q))
          ENDDO
...
!                MULTDN = UT_PMULT_UD(K,UM,UT) / ATERM_SAVE(K,N) ; IGAM = ONE / GAMMA_M(K,N)
                ...
                DO Q = 1, NV_PARAMETERS
                  ...
                  L_MULTDN(Q) = ( L_FIRST - LP_UT_EMULT_UP(LUM,UT,NV,IB,Q) - L_GAMMA * MULTDN ) * IGAM
                ENDDO
              endif
...
!mick mod 1/5/2021 - removed neg. sign from L_FIRST for consistency with PARTLAYER_STERM_UP
              ...
              DO Q = 1, NV_PARAMETERS
                ...
                L_MULTUP(Q) = ( L_FIRST + LP_UT_EMULT_UP(LUM,UT,NV,IB,Q) - L_GAMMA * MULTUP ) * IGAM
              ENDDO
...
!                MULTDN = UT_PMULT_UD(K,UM,UT) / ATERM_SAVE(K,N) ; IGAM = ONE / GAMMA_M(K,N)
                ...
                DO Q = 1, NV_PARAMETERS
                  L_GAMMA  = LP_AVERAGE_SECANT(N,NV,IB,Q)
                  L_FIRST  = L_ITRANS(Q) * UT_HMULT_UD(K,UM,UT)
                  L_MULTDN(Q) = ( L_FIRST - LP_UT_EMULT_UP(LUM,UT,NV,IB,Q) - L_GAMMA * MULTDN ) * IGAM
                ENDDO
              endif
...
!mick mod 1/5/2021 - removed neg. sign from L_FIRST for consistency with PARTLAYER_STERM_UP
              ...
              DO Q = 1, NV_PARAMETERS
                L_GAMMA  = LP_AVERAGE_SECANT(N,NV,IB,Q)
                L_FIRST  = L_ITRANSWDEL(Q) * UT_HMULT_UU(K,UM,UT)
                L_MULTUP(Q) = ( L_FIRST + LP_UT_EMULT_UP(LUM,UT,NV,IB,Q) - L_GAMMA * MULTUP ) * IGAM
              ENDDO
...
!  End clause for N=NV versus N>NV
...
!  1/31/21. Section re-written to cover  both options in post-processing masks
      ...
      IF ( .NOT. DO_MSMODE_VLIDORT ) THEN
        IF ( N.EQ.NV ) THEN
          DO LUM = 1, N_PPSTREAMS
            ...
            DO O1 = 1, NSTOKES
              DO Q = 1, NV_PARAMETERS
                SP = L_UPAR_UP_1(UM,O1,N,Q) *   UT_EMULT_UP(LUM,UT,IB) + &
                       UPAR_UP_1(UM,O1,N)   * LP_UT_EMULT_UP(LUM,UT,NV,IB,Q)
                L_LAYERSOURCE(UM,O1,Q) = L_LAYERSOURCE(UM,O1,Q) + SP
              ENDDO
            ENDDO
          ENDDO
        ELSE
          DO LUM = 1, N_PPSTREAMS
            ...
            DO O1 = 1, NSTOKES
              DO Q = 1, NV_PARAMETERS
                SP =  UPAR_UP_1(UM,O1,N) * LP_UT_EMULT_UP(LUM,UT,NV,IB,Q)
                L_LAYERSOURCE(UM,O1,Q) = L_LAYERSOURCE(UM,O1,Q) + SP
              ENDDO
            ENDDO
          ENDDO
...
!  1/31/21. Version 2.8.3. New Help variables for the Green's function solution
      ...
      DOUBLE PRECISION :: ITRANS, WDEL, ITRANSWDEL, MULTDN, MULTUP, EPS, PARTA, YFAC
      DOUBLE PRECISION :: IGAM, LAM, SM, FAC1, FAC2
      DOUBLE PRECISION :: L_FIRST, L_PARTA, L_KEG, L_LAM, L_GAMMA, L_WDEL
      DOUBLE PRECISION :: L_MULTDN(MAX_ATMOSWFS)  , L_MULTUP(MAX_ATMOSWFS)
      DOUBLE PRECISION :: L_ITRANS(MAX_ATMOSWFS)  , L_ITRANSWDEL(MAX_ATMOSWFS)
      DOUBLE PRECISION :: L_PMULT_UP(MAX_ATMOSWFS), L_PMULT_DN(MAX_ATMOSWFS)
      DOUBLE PRECISION :: SPARG(MAXSTOKES,MAX_ATMOSWFS)
...
!   Very important to zero output terms (bug solved 12/29/05)
!    -- 1/31/21. Version 2.8.3. Note use of post-processing mask (et seq. in this and other subroutines)
...
!  add the MS particular solution 
!     -- Special case when N = NV, needs the LP_UPAR_DN_2 linearization
        ...
        IF ( N.EQ.NV ) THEN
          DO LUM = 1, N_PPSTREAMS
            ...
            DO O1 = 1, NSTOKES
              DO Q = 1, NV_PARAMETERS
                SP = LP_UPAR_DN_2(UM,O1,N,NV,Q) *   UT_EMULT_DN(LUM,UT,IB) + &
                        UPAR_DN_2(UM,O1,N)      * LP_UT_EMULT_DN(LUM,UT,NV,IB,Q)
                L_LAYERSOURCE(UM,O1,Q) = L_LAYERSOURCE(UM,O1,Q) + SP
              ENDDO
            ENDDO
          ENDDO
...
!  This section for N = NV
!  -----------------------
!mick fix 1/5/2021 - changed K to NV in IF condition below
...
!                MULTDN = UT_PMULT_DD(K,UM,UT) / ATERM_SAVE(K,N) ; IGAM = ONE / GAMMA_M(K,N)
                ...
                DO Q = 1, NV_PARAMETERS
                  ...
                  L_MULTDN(Q) = ( L_FIRST - LP_UT_EMULT_DN(LUM,UT,NV,IB,Q) - L_GAMMA * MULTDN ) * IGAM
                ENDDO
              endif
...
!mick mod 1/5/2021 - removed neg. sign from L_FIRST for consistency with PARTLAYER_STERM_DN
              ...
              DO Q = 1, NV_PARAMETERS
                ...
                L_MULTUP(Q) = ( L_FIRST + LP_UT_EMULT_DN(LUM,UT,NV,IB,Q) - L_GAMMA * MULTUP ) * IGAM
              ENDDO
...
!                MULTDN = UT_PMULT_DD(K,UM,UT) / ATERM_SAVE(K,N) ; IGAM = ONE / GAMMA_M(K,N)
                ...
                DO Q = 1, NV_PARAMETERS
                  L_GAMMA  = LP_AVERAGE_SECANT(N,NV,IB,Q)
                  L_FIRST  = L_ITRANS(Q) * UT_HMULT_DD(K,UM,UT)
                  L_MULTDN(Q) = ( L_FIRST - LP_UT_EMULT_DN(LUM,UT,NV,IB,Q) - L_GAMMA * MULTDN ) * IGAM
                ENDDO
              endif
...
!mick mod 1/5/2021 - removed neg. sign from L_FIRST for consistency with PARTLAYER_STERM_DN
              ...
              DO Q = 1, NV_PARAMETERS
                L_GAMMA  = LP_AVERAGE_SECANT(N,NV,IB,Q)
                L_FIRST  = L_ITRANSWDEL(Q) * UT_HMULT_DU(K,UM,UT)
                L_MULTUP(Q) = ( L_FIRST + LP_UT_EMULT_DN(LUM,UT,NV,IB,Q) - L_GAMMA * MULTUP ) * IGAM
              ENDDO
...
!  End clause for N=NV against N>NV
      ...
      IF ( .NOT. DO_MSMODE_VLIDORT ) THEN
            ...
            DO O1 = 1, NSTOKES
              DO Q = 1, NV_PARAMETERS
                SP = L_UPAR_DN_1(UM,O1,N,Q) *   UT_EMULT_DN(LUM,UT,IB) + &
                       UPAR_DN_1(UM,O1,N)   * LP_UT_EMULT_DN(LUM,UT,NV,IB,Q)
                L_LAYERSOURCE(UM,O1,Q) = L_LAYERSOURCE(UM,O1,Q) + SP
              ENDDO
            ENDDO
          ENDDO
        ELSE
          DO LUM = 1, N_PPSTREAMS
            ...
            DO O1 = 1, NSTOKES
              DO Q = 1, NV_PARAMETERS
                SP =  UPAR_DN_1(UM,O1,N) * LP_UT_EMULT_DN(LUM,UT,NV,IB,Q)
                L_LAYERSOURCE(UM,O1,Q) = L_LAYERSOURCE(UM,O1,Q) + SP
              ENDDO
            ENDDO
          ENDDO
...
!  module, dimensions and numbers
      ...
      USE VLIDORT_pars_m, only : MAXSTREAMS, MAXSTOKES, MAX_PARTLAYERS, MAXEVALUES, &
                                 MAXLAYERS, MAXBEAMS, MAX_ATMOSWFS, ZERO, ONE, TAYLOR_SMALL, LDU
...
!  Beam index,  offgrid indices, number of parameters
      ...
      INTEGER  , intent(in)  :: IB, N, UT, NV, NV_PARAMETERS
...
!  Linearized Green functions multipliers for off-grid optical depths
      ...
      DOUBLE PRECISION, intent(inout) :: LP_UT_GMULT_UP(MAXEVALUES,MAX_PARTLAYERS,MAXLAYERS,MAX_ATMOSWFS)
      DOUBLE PRECISION, intent(inout) :: LP_UT_GMULT_DN(MAXEVALUES,MAX_PARTLAYERS,MAXLAYERS,MAX_ATMOSWFS)
...
!mick fix 1/5/2021 - initialize all K entries of LP_UT_GMULT_UP & LP_UT_GMULT_DN
!                    whether or not beam is cut off

      !IF ( N .GT. BEAM_CUTOFF(IB) ) THEN
      !  DO K = 1, K_REAL(N)
      !    DO Q = 1, NV_PARAMETERS
      !      LP_UT_GMULT_UP(K,UT,NV,Q) = ZERO
      !      LP_UT_GMULT_DN(K,UT,NV,Q) = ZERO
      !    ENDDO
      !  ENDDO
      !  RETURN
      !ENDIF
      ...
      DO Q = 1, NV_PARAMETERS
        LP_UT_GMULT_UP(:,UT,NV,Q) = ZERO
        LP_UT_GMULT_DN(:,UT,NV,Q) = ZERO
      ENDDO
      IF ( N .GT. BEAM_CUTOFF(IB) ) RETURN
...
!  Layer constant terms
      ...
      ITRANS = INITIAL_TRANS(N,IB)
      DO Q = 1, NV_PARAMETERS
        L_ITRANS(Q) = LP_INITIAL_TRANS(N,NV,IB,Q)
        L_WX(Q)     = LP_T_UTDN_MUBAR(UT,NV,IB,Q)
        L_WDEL(Q)   = LP_T_DELT_MUBAR(N,NV,IB,Q)
      ENDDO
...
!   -- Have to introduce Taylor series calculation of MULTDN, as well as L_MULTDN
          ...
          IF ( ABS(GAMMA_M(K,N)) .LT. TAYLOR_SMALL ) THEN
            ...
            LAM   = AVERAGE_SECANT(N,IB)
            DO Q = 1, NV_PARAMETERS
               L_LAM  = LP_AVERAGE_SECANT(N,NV,IB,Q) ; L_KEG  = L_KEIGEN(K,N,Q)
               L_PARTA = L_DELTAU_VERT(Q,N) * PARTA ! Input is single normalized
            ...
            ENDDO
          ELSE
            IGAM = ONE / GAMMA_M(K,N) ; MULTDN =  ( ZX_DN - WX ) * IGAM
            DO Q = 1, NV_PARAMETERS
...
!   Rob Change 1/15/21. replace this do-loop using regular derivatives (non-logarithmic)
!          DO Q = 1, NV_PARAMETERS
!            LP_UT_GMULT_DN(K,UT,NV,Q) = ITA * L_MULTDN(Q) + GDN * ( L_ITRANS(Q) + L_ATERM_SAVE(K,N,Q) )
!            LP_UT_GMULT_UP(K,UT,NV,Q) = ITB * L_MULTUP(Q) + GUP * ( L_ITRANS(Q) + L_BTERM_SAVE(K,N,Q) )
!          ENDDO
          DO Q = 1, NV_PARAMETERS

!mick fix 1/5/2021 - in reformulated 2nd line, changed LP_UT_GMULT suffix from DN to UP
!                  - replaced GDN and GUP with MULTDN * ATERM_SAVE(K,N) and MULTUP * BTERM_SAVE(K,N), respectively

            !LP_UT_GMULT_DN(K,UT,NV,Q) = ITA * L_MULTDN(Q) + GDN * L_ITRANS(Q) + MULTDN * ITRANS * L_ATERM_SAVE(K,N,Q)  
            !!LP_UT_GMULT_DN(K,UT,NV,Q) = ITB * L_MULTUP(Q) + GUP * L_ITRANS(Q) + MULTUP * ITRANS * L_BTERM_SAVE(K,N,Q) 
            !LP_UT_GMULT_UP(K,UT,NV,Q) = ITB * L_MULTUP(Q) + GUP * L_ITRANS(Q) + MULTUP * ITRANS * L_BTERM_SAVE(K,N,Q)

            LP_UT_GMULT_DN(K,UT,NV,Q) = ITA * L_MULTDN(Q) + MULTDN * ATERM_SAVE(K,N) * L_ITRANS(Q) &
                                      + MULTDN * ITRANS * L_ATERM_SAVE(K,N,Q)  
            LP_UT_GMULT_UP(K,UT,NV,Q) = ITB * L_MULTUP(Q) + MULTUP * BTERM_SAVE(K,N) * L_ITRANS(Q) &
...
!  Complete the multipliers
          ...
          ITB = ITRANS * BTERM_SAVE(K,N); GUP = UT_GMULT_UP(K,UT)
          DO Q = 1, NV_PARAMETERS
            LP_UT_GMULT_DN(K,UT,NV,Q) = ITA * L_MULTDN(Q) + GDN * L_ITRANS(Q)
            LP_UT_GMULT_UP(K,UT,NV,Q) = ITB * L_MULTUP(Q) + GUP * L_ITRANS(Q)
          ENDDO
...
!     -- New arguments: multipliers UT_GMULT_UP, UT_GMULT_DN, LP_UT_GMULT_UP, LP_UT_GMULT_DN
...
!    -- 1/31//21. Version 2.8.3. Added for the Green's function calculation
      ...
      DOUBLE PRECISION, intent (in) :: LP_UT_GMULT_UP(MAXEVALUES,MAX_PARTLAYERS,MAXLAYERS,MAX_ATMOSWFS)
      DOUBLE PRECISION, intent (in) :: LP_UT_GMULT_DN(MAXEVALUES,MAX_PARTLAYERS,MAXLAYERS,MAX_ATMOSWFS)
...
!    --- Only present if N = NV
      ...
      IF ( DO_INCLUDE_THERMEMISS .and. N.EQ.NV ) THEN
        O1 = 1
        DO I = 1, NSTREAMS
          I1 = I + NSTREAMS
          DO Q = 1, NV_PARAMETERS
            ...
            DO I = 1, NSTREAMS
              ...
              DO O1 = 1, NSTOKES
                DO Q = 1, NV_PARAMETERS
                  ...
                  ENDIF
                  SPAR = L_SUNP * T_UTDN_MUBAR(UT,IB) + SUNP * LP_T_UTDN_MUBAR(UT,NV,IB,Q)
                  QATMOSWF_F(Q,UTA,I,O1) = QATMOSWF_F(Q,UTA,I,O1) + FMULT * SPAR
                ENDDO
              ENDDO
            ENDDO
...
!  solution for N being the varying layer NV, or N > NV
            ...
            DO I = 1, NSTREAMS
              DO Q = 1, NV_PARAMETERS
                SPAR = DOT_PRODUCT (    UT_GMULT_DN(1:NSTKS_NSTRMS,UT),      L_SOLB_XNEG(I,O1,1:NSTKS_NSTRMS,N,Q) ) &
                     + DOT_PRODUCT ( LP_UT_GMULT_DN(1:NSTKS_NSTRMS,UT,NV,Q),   SOLB_XNEG(I,O1,1:NSTKS_NSTRMS,N)   ) &
                     + DOT_PRODUCT (    UT_GMULT_UP(1:NSTKS_NSTRMS,UT),      L_SOLA_XPOS(I,O1,1:NSTKS_NSTRMS,N,Q) ) &
                     + DOT_PRODUCT ( LP_UT_GMULT_UP(1:NSTKS_NSTRMS,UT,NV,Q),   SOLA_XPOS(I,O1,1:NSTKS_NSTRMS,N)   )
        ...
        ELSE IF ( N.GT.NV ) THEN
          DO O1 = 1, NSTOKES
            DO I = 1, NSTREAMS
              DO Q = 1, NV_PARAMETERS
                SPAR = DOT_PRODUCT ( LP_UT_GMULT_DN(1:NSTKS_NSTRMS,UT,NV,Q),   SOLB_XNEG(I,O1,1:NSTKS_NSTRMS,N)   ) &
                     + DOT_PRODUCT ( LP_UT_GMULT_UP(1:NSTKS_NSTRMS,UT,NV,Q),   SOLA_XPOS(I,O1,1:NSTKS_NSTRMS,N)   )
                IF ( O1.gt.2) SPAR = -SPAR
                QATMOSWF_F(Q,UTA,I,O1) = QATMOSWF_F(Q,UTA,I,O1) + FMULT * SPAR
              ENDDO
        ...
        IF ( DO_INCLUDE_THERMEMISS ) THEN
          ...
          IF ( N.GT.NV .or. VAREX ) THEN
            DO I = 1, NSTREAMS
              DO O1 = 1, NSTOKES
                DO Q = 1, NV_PARAMETERS
                  SUNP   = WUPPER(I,O1,N) ; L_SUNP = L_WUPPER(I,O1,N,Q)
                  IF ( O1 .eq. 1 ) THEN
        ...
        ELSE
          IF ( N.GE.NV ) THEN
            DO I = 1, NSTREAMS
              DO O1 = 1, NSTOKES
                DO Q = 1, NV_PARAMETERS
                  SPAR = L_WUPPER(I,O1,N,Q) * T_UTDN_MUBAR(UT,IB) + WUPPER(I,O1,N) * LP_T_UTDN_MUBAR(UT,NV,IB,Q)
                  QATMOSWF_F(Q,UTA,I,O1) = QATMOSWF_F(Q,UTA,I,O1) + FMULT * SPAR
                ENDDO
              ENDDO
            ENDDO
...
!  Solution for N being the varying layer NV, or N > NV
            ...
            DO I = 1, NSTREAMS
              DO Q = 1, NV_PARAMETERS
                SPAR = DOT_PRODUCT (    UT_GMULT_UP(1:NSTKS_NSTRMS,UT),      L_SOLB_XNEG(I,O1,1:NSTKS_NSTRMS,N,Q) ) &
                     + DOT_PRODUCT ( LP_UT_GMULT_UP(1:NSTKS_NSTRMS,UT,NV,Q),   SOLB_XNEG(I,O1,1:NSTKS_NSTRMS,N)   ) &
                     + DOT_PRODUCT (    UT_GMULT_DN(1:NSTKS_NSTRMS,UT),      L_SOLA_XPOS(I,O1,1:NSTKS_NSTRMS,N,Q) ) &
                     + DOT_PRODUCT ( LP_UT_GMULT_DN(1:NSTKS_NSTRMS,UT,NV,Q),   SOLA_XPOS(I,O1,1:NSTKS_NSTRMS,N)   )
                IF ( O1.gt.2) SPAR = -SPAR
                QATMOSWF_F(Q,UTA,I,O1) = QATMOSWF_F(Q,UTA,I,O1)  + FMULT * SPAR
              ENDDO
            ENDDO
        ...
        ELSE IF ( N.GT.NV ) THEN
          DO O1 = 1, NSTOKES
            DO I = 1, NSTREAMS
              DO Q = 1, NV_PARAMETERS
                SPAR = DOT_PRODUCT ( LP_UT_GMULT_UP(1:NSTKS_NSTRMS,UT,NV,Q),   SOLB_XNEG(I,O1,1:NSTKS_NSTRMS,N)   ) &
                     + DOT_PRODUCT ( LP_UT_GMULT_DN(1:NSTKS_NSTRMS,UT,NV,Q),   SOLA_XPOS(I,O1,1:NSTKS_NSTRMS,N)   )
                IF ( O1.gt.2) SPAR = -SPAR
                QATMOSWF_F(Q,UTA,I,O1) = QATMOSWF_F(Q,UTA,I,O1)  + FMULT * SPAR
              ENDDO
...
!  End Module
      ...
      END MODULE vlidort_lp_PostProcessing_m
