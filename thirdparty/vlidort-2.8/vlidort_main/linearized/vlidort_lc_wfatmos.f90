...
!  1/31/21. Version 2.8.3. Quadrature and sourceterm routines moved to new module LC_POSTPROCESSING
!  1/31/21. Version 2.8.3. LCS Convergence routines moved to new module LCS_CONVERGE
...
!  1/31/21. Version 2.8.3. Dependency for Quadrature and sourceterm routines
      ...
      USE vlidort_lc_PostProcessing_m
...
!   1/31/21. Version 2.8.3. Only the TOA/BOA source routines are now private
      ...
      SUBROUTINE UPUSER_COLUMNWF ( &
        ...
        N_PPSTREAMS, PPSTREAM_MASK, N_USER_LEVELS, NV_PARAMETERS,                     & ! Input numbers (basic)
        ...
        LC_INITIAL_TRANS, LC_AVERAGE_SECANT, LC_T_DELT_MUBAR, L_T_DELT_DISORDS,          & ! Linearized BeamParm/DODTrans
        ...
        L_HMULT_1, L_HMULT_2, LC_EMULT_UP, L_UT_HMULT_UU, L_UT_HMULT_UD, LC_UT_EMULT_UP, & ! Linearized multipliers
        L_UPAR_UP_1, LC_UPAR_UP_2, L_ATERM_SAVE, L_BTERM_SAVE, L_WLOWER, NCON, PCON,     & ! Linearized PI Solutions
...
!    -- 1/31/21. Version 2.8.3. New output
      ...
      DOUBLE PRECISION, INTENT (INOUT) :: LC_SURF_MSSTS_F  ( MAXBEAMS, MAXSTOKES, MAX_ATMOSWFS )
      DOUBLE PRECISION, INTENT (INOUT) :: LC_LAYER_MSSTS_F ( MAXBEAMS, MAXSTOKES, MAXLAYERS, MAX_ATMOSWFS )
...
!rob & mick fix 1/5/2021 - added this zeroing
      ...
      DO Q = 1, NV_PARAMETERS
        DO O1 = 1, ECHT_NSTOKES
          LC_LAYER_MSSTS_F(IB,O1,1:NLAYERS,Q) = ZERO
          LC_SURF_MSSTS_F (IB,O1,Q)           = ZERO
        ENDDO
      ENDDO
...
!     -- 1/31/21. Version 2.8.3. Use post-processing mask input, define BRDF arrays locally.
        ...
        CALL GET_LC_BOASOURCE ( &
          ...
          DO_USER_STREAMS, DO_INCLUDE_MVOUTPUT, DO_INCLUDE_SURFACE, DO_DBCORR,   & ! Input flags
          DO_LAMBERTIAN_SURFACE, DO_INCLUDE_DIRECTRF, DO_INCLUDE_DIRECTSL,       & ! Input flags
          FOURIER, IBEAM, NSTOKES, NSTREAMS, NLAYERS, NV_PARAMETERS,             & ! Input Numbers/Indices
          N_PPSTREAMS, PPSTREAM_MASK, QUAD_WEIGHTS, QUAD_STRMWTS, MUELLER_INDEX, & ! Input bookkeeping
          DELTAU_SLANT, SURFACE_FACTOR, ALBEDO, BRDF_F, USER_BRDF_F,             & ! Input Surface/Deltau
...
!    -- NOTE, MSSTS only available for OBSERVATION GEOMETRY !!!!!!!!!!!!!!!!!!!!!!!!
        ...
        IF ( DO_MSSTS .and. DO_OBSERVATION_GEOMETRY ) then
          DO O1 = 1, NSTOKES
            DO Q = 1, NV_PARAMETERS
              LC_SURF_MSSTS_F(IB,O1,Q) = FLUX_MULTIPLIER * L_BOA_MSSOURCE(IB,O1,Q)
            ENDDO
          ENDDO
        ENDIF
...
!    -- Introduce Other inputs LC_AVERAGE_SECANT, SIGMA_P, L_ATERM_SAVE, L_BTERM_SAVE

            CALL LC_WHOLELAYER_STERM_UP ( &
              DO_SOLAR_SOURCES, DO_INCLUDE_THERMEMISS, DO_THERMAL_TRANSONLY,                   & ! Input flags
              DO_CLASSICAL_SOLUTION, DO_MSMODE_VLIDORT, SFLAG, FOURIER,                        & ! Input flags/Fourier
              N, IBEAM, NV_PARAMETERS, NSTOKES, N_PPSTREAMS, PPSTREAM_MASK,                    & ! Input numbers
...
!    -- NOTE, MSSTS only available for OBSERVATION GEOMETRY !!!!!!!!!!!!!!!!!!!!!!!!1
            ...
            IF ( DO_MSSTS .and. DO_OBSERVATION_GEOMETRY) THEN
              DO O1 = 1, NSTOKES
                DO Q = 1, NV_PARAMETERS
                  LC_LAYER_MSSTS_F(IB,O1,N,Q) = FLUX_MULTIPLIER * L_LAYER_SOURCE(IB,O1,Q)
                ENDDO
              ENDDO
            ENDIF
...
!    -- Introduce Other inputs SIGMA_P, LC_AVERAGE_SECANT, L_ATERM_SAVE, L_BTERM_SAVE

            CALL LC_PARTLAYER_STERM_UP ( &
              DO_SOLAR_SOURCES, DO_INCLUDE_THERMEMISS, DO_THERMAL_TRANSONLY,                & ! Input flags
              DO_CLASSICAL_SOLUTION, DO_MSMODE_VLIDORT, SFLAG, FOURIER, IBEAM,              & ! Input flags/numbers
              N, UT, NV_PARAMETERS, NSTOKES, N_PPSTREAMS, PPSTREAM_MASK, TAYLOR_ORDER,      & ! Input numbers
...
!
      ...
      SUBROUTINE DNUSER_COLUMNWF ( &
        ...
        ECHT_NSTOKES, NSTOKES, NLAYERS, N_USER_LEVELS, NV_PARAMETERS, N_PPSTREAMS, PPSTREAM_MASK, & ! Input flags/numbers (basic)
        ...
        L_T_DELT_USERM, L_T_UTDN_USERM, LC_INITIAL_TRANS, LC_AVERAGE_SECANT, LC_T_DELT_MUBAR, & ! Linearized BeamParm/Trans
        L_KEIGEN, L_UHOM_DNDN, L_UHOM_DNUP, L_LAYER_TSUP_DN, L_LAYER_TSUP_UTDN,               & ! Linearized Homog/Thermal
        L_HMULT_1, L_HMULT_2, LC_EMULT_DN, L_UT_HMULT_DU, L_UT_HMULT_DD, LC_UT_EMULT_DN,      & ! Linearized multipliers
        L_UPAR_DN_1, LC_UPAR_DN_2, L_ATERM_SAVE, L_BTERM_SAVE, NCON, PCON,                    & ! Linearized PI Solutions
...
!  Local arrays
      ...
      DOUBLE PRECISION :: L_CUMUL_SOURCE ( MAX_USER_STREAMS, MAXSTOKES, MAX_ATMOSWFS )
      DOUBLE PRECISION :: LC_TOA_SOURCE  ( MAX_USER_STREAMS, MAXSTOKES, MAX_ATMOSWFS )
      DOUBLE PRECISION :: L_LAYER_SOURCE ( MAX_USER_STREAMS, MAXSTOKES, MAX_ATMOSWFS )
...
!     -- 1/31/21. Version 2.8.3. Use post-processing mask input
        ...
        CALL GET_LC_TOASOURCE ( &
           N_PPSTREAMS, PPSTREAM_MASK, NSTOKES, IBEAM, NV_PARAMETERS, LC_TOA_SOURCE )
...
!     -- 1/31/21. Version 2.8.3. Use post-processing mask input
        ...
        DO LUM = 1, N_PPSTREAMS
          ...
          DO O1 = 1, NSTOKES
            DO Q = 1, NV_PARAMETERS
              L_CUMUL_SOURCE(UM,O1,Q) = LC_TOA_SOURCE(UM,O1,Q)
            ENDDO
          ENDDO
        ENDDO
...
!    -- Introduce Other inputs LC_AVERAGE_SECANT, SIGMA_M, L_ATERM_SAVE, L_BTERM_SAVE
...
!    -- NOTE, MSSTS only available for OBSERVATION GEOMETRY !!!!!!!!!!!!!!!!!!!!!!!!
            ...
            IF ( DO_MSSTS .and. DO_OBSERVATION_GEOMETRY ) THEN
              DO O1 = 1, NSTOKES
                DO Q = 1, NV_PARAMETERS
                  LC_LAYER_MSSTS_F(IB,O1,N,Q) = FLUX_MULT * L_LAYER_SOURCE(IB,O1,Q)
                ENDDO
              ENDDO
            ENDIF
...
!    -- Introduce Post-processing Mask N_PPSTREAMS, PPSTREAM_MASK
!    -- Introduce Green's function inputs ATERM_SAVE, BTERM_SAVE, GAMMA_P, GAMMA_M, UT_PMULT_UU, UT_PMULT_UD
!    -- Introduce Other inputs SIGMA_M, LC_AVERAGE_SECANT, L_ATERM_SAVE, L_BTERM_SAVE

            CALL LC_PARTLAYER_STERM_DN ( &
              DO_SOLAR_SOURCES, DO_INCLUDE_THERMEMISS, DO_THERMAL_TRANSONLY,                & ! Input flags
              DO_CLASSICAL_SOLUTION, DO_MSMODE_VLIDORT, SFLAG, FOURIER,                     & ! Input flags/numbers
              IBEAM, N, UT, NV_PARAMETERS, NSTOKES, N_PPSTREAMS, PPSTREAM_MASK,             & ! Input numbers
              TAYLOR_ORDER, PARTAU_VERT, L_DELTAU_VERT, USER_SECANTS, T_UTDN_USERM,         & ! Input Optical/Usertrans
              BEAM_CUTOFF, INITIAL_TRANS, T_DELT_MUBAR, T_UTDN_MUBAR,                       & ! Input Beam stuff
            ...
            DO LUM = 1, N_PPSTREAMS
               ...
               DO O1 = 1, NSTOKES
                  DO Q = 1, NV_PARAMETERS
                     ...
                     COLUMNWF_F(Q,UTA,LUM,IB,O1,DNIDX) = FLUX_MULT * L_FINAL_SOURCE
                  ENDDO
               ENDDO
            ENDDO
...
!    -- 1/31/21. Version 2.8.3. use Post-processing Mask N_PPSTREAMS, PPSTREAM_MASK
          ...
          IF ( DO_USER_STREAMS ) THEN
             DO LUM = 1, N_PPSTREAMS
                ...
                DO O1 = 1, NSTOKES
                   DO Q = 1, NV_PARAMETERS
                      COLUMNWF_F(Q,UTA,LUM,IB,O1,DNIDX)  = FLUX_MULT * L_CUMUL_SOURCE(UM,O1,Q)
                   ENDDO
                ENDDO
             ENDDO
          ENDIF
...
!  Beam, Stokes
      ...
      INTEGER  , intent(in)  :: IBEAM, NSTOKES, NV_PARAMETERS

!  Output

      DOUBLE PRECISION, INTENT (OUT) :: LC_TOA_SOURCE ( MAX_USER_STREAMS, MAXSTOKES, MAX_ATMOSWFS )
...
!  ------------------------------
      ...
      DO LUM = 1, N_PPSTREAMS
         ...
         DO Q = 1, NV_PARAMETERS
            LC_TOA_SOURCE(UM,1:NSTOKES,Q) = ZERO
         ENDDO
      ENDDO
...
!  Finish
      ...
      SUBROUTINE MIFLUX_COLUMNWF ( &
        ...
        DO_SOLAR_SOURCES, DO_INCLUDE_THERMEMISS, DO_THERMAL_TRANSONLY,    & ! Input flags
        IBEAM, NV_PARAMETERS, NSTOKES, NSTREAMS, NSTKS_NSTRMS, NLAYERS,   & ! Input numbers
...
!  Beam parameterization
!    -- 1/31/21. Version 2.8.3. LC_AVERAGE_SECANT added
...
!    -- 1/31/21. Version 2.8.3. Added for the Green's function calculation
      ...
      DOUBLE PRECISION :: LC_UT_GMULT_UP(MAXEVALUES,MAX_PARTLAYERS,MAX_ATMOSWFS)
      DOUBLE PRECISION :: LC_UT_GMULT_DN(MAXEVALUES,MAX_PARTLAYERS,MAX_ATMOSWFS)
        ...
        DO UTA = 1, N_USER_LEVELS
          ...
          IF ( PARTLAYERS_OUTFLAG(UTA) ) THEN
            ...
            CALL LC_QUAD_GFUNCMULT &
              ( IBEAM, UT, N, NV_PARAMETERS, TAYLOR_ORDER, PARTAU_VERT, L_DELTAU_VERT,   & ! input
...
!     -- New Greens' multipliers UT_GMULT_UP, UT_GMULT_DN, LC_UT_GMULT_UP, LC_UT_GMULT_DN

              CALL QUADCOLUMNWF_OFFGRID_UP ( &
                DO_SOLAR_SOURCES, DO_INCLUDE_THERMEMISS, DO_THERMAL_TRANSONLY, DO_CLASSICAL, & ! Input flags
                N, UTA, UT, IBEAM, NV_PARAMETERS, NSTOKES, NSTREAMS, NLAYERS, NSTKS_NSTRMS,  & ! Input numbers
...
!     -- New Greens' multipliers UT_GMULT_UP, UT_GMULT_DN, LC_UT_GMULT_UP, LC_UT_GMULT_DN
              ...
              CALL QUADCOLUMNWF_OFFGRID_DN ( &
                DO_SOLAR_SOURCES, DO_INCLUDE_THERMEMISS, DO_THERMAL_TRANSONLY, DO_CLASSICAL,     & ! Input flags
                N, UTA, UT, IBEAM, NV_PARAMETERS, NSTOKES, NSTREAMS, NSTKS_NSTRMS,               & ! Input numbers
