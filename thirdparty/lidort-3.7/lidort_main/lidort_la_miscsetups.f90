...
!  module, dimensions and numbers
      ...
      USE LIDORT_pars, only : MAXMOMENTS_INPUT, MAXLAYERS, MAXMOMENTS,  &
         ...
         DO K = 1, N
          IF ( LAYER_VARY_FLAG(K) ) THEN
           DO Q = 1, LAYER_VARY_NUMBER(K)
            L_1 = L_TRUNC_FACTOR(Q,K) *   OMEGA_TOTAL_INPUT(K)   &
                  + TRUNC_FACTOR(K)   * L_OMEGA_TOTAL_INPUT(Q,K)
            L_2 =  - L_1 *   DELTAU_VERT_INPUT(K) +  &
                 FAC1(K) * L_DELTAU_VERT_INPUT(Q,K)
            DO IB = 1, NBEAMS
...
!  module, dimensions and numbers
      ...
      USE LIDORT_pars, only : MAXLAYERS, MAX_PARTLAYERS, MAX_USER_STREAMS,  &
...
end module lidort_la_miscsetups
