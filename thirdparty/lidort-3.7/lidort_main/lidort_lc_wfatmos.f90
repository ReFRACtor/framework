...
SUBROUTINE MIFLUX_COLUMNWF                                           &
           ( DO_UPWELLING, DO_DNWELLING, DO_INCLUDE_DIRECTBEAM,      & ! Input
             IB, K_PARAMETERS, NSTREAMS, N_USER_LEVELS, FLUX_FACTOR, & ! Input (Remove thread)
             PARTLAYERS_OUTFLAG, PARTLAYERS_OUTINDEX,                & ! Input
             UTAU_LEVEL_MASK_DN, PARTLAYERS_LAYERIDX,                & ! Input
      ...
      IF ( DO_UPWELLING ) THEN
        DO UTA = 1, N_USER_LEVELS
         DO Q = 1, K_PARAMETERS
          ...
          ENDDO
          MINT_COLUMNWF(Q,UTA,IB,UPIDX) = SM * HALF
          FLUX_COLUMNWF(Q,UTA,IB,UPIDX) = SF * PI2
         ENDDO
        ENDDO
      ENDIF
...
!  Diffuse term contribution
        ...
        DO UTA = 1, N_USER_LEVELS
         DO Q = 1, K_PARAMETERS
          ...
          ENDDO
          MINT_COLUMNWF(Q,UTA,IB,DNIDX) = SM * HALF
          FLUX_COLUMNWF(Q,UTA,IB,DNIDX) = SF * PI2
         ENDDO
        ENDDO
...
!    L_INITIAL_TRANS is a logarithmic derivative
            ...
            IF ( N .LE. LAYER_PIS_CUTOFF(IB) ) THEN
              ...
              DO Q = 1, K_PARAMETERS
                ...
                MINT_COLUMNWF(Q,UTA,IB,DNIDX) =  &
                      MINT_COLUMNWF(Q,UTA,IB,DNIDX) + L_DIRECT_MEANI
                FLUX_COLUMNWF(Q,UTA,IB,DNIDX) =  &
                      FLUX_COLUMNWF(Q,UTA,IB,DNIDX) + L_DIRECT_FLUX
              ENDDO
            ENDIF
...
!  Add direct beam if flagged
      ...
      IF ( DO_INCLUDE_DIRECTBEAM .AND. DO_USER_STREAMS ) THEN
        IF (.not. DO_OBSERVATION_GEOMETRY ) THEN
          ...
          ENDDO
        ELSE
          FAC = - USER_DIRECT_BEAM(LUM,IB)
          DO Q = 1, K_PARAMETERS
            L_BEAM = ZERO
            DO K1 = 1, N
              L_BEAM = L_BEAM + L_DELTAU_VERT(Q,K1) * DELTAU_SLANT(N,K1,IB)
            ENDDO
            L_BOA_DBSOURCE(IB,Q) = L_BEAM * FAC
          ENDDO
        ENDIF
      ENDIF
...
SUBROUTINE LIDORT_LC_CONVERGE                                    &
           ( DO_UPWELLING, DO_SS_EXTERNAL, DO_SSFULL,            & ! input
             ...
             IBEAM, FOURIER_COMPONENT, UMOFF,                    & ! input (Remove thread)
...
SUBROUTINE LIDORT_LC_CONVERGE_OBSGEO                                  &
           ( DO_UPWELLING, DO_SS_EXTERNAL, DO_SSFULL,                 & ! input
             ...
             AZMFAC, N_USER_LEVELS, IBEAM, FOURIER_COMPONENT,         & ! input (Remove thread)
             N_DIRECTIONS, WHICH_DIRECTIONS, N_TOTALCOLUMN_WFS,       & ! input
             COLUMNWF_F, COLUMNWF_SS, COLUMNWF_DB,                    & ! input
             COLUMNWF )                                                 ! output
        ...
        IF ( DO_SSFULL .OR. &
          ...
          DO Q = 1, N_TOTALCOLUMN_WFS
            DO IDIR = 1, N_DIRECTIONS
              W = WHICH_DIRECTIONS(IDIR)
              DO UT = 1, N_USER_LEVELS
                COLUMNWF(Q,UT,IBEAM,W) = &
                  COLUMNWF(Q,UT,IBEAM,W) + COLUMNWF_SS(Q,UT,IBEAM,W)
              ENDDO
            ENDDO
          ENDDO
        ENDIF
...
!  New 15 March 2012, Introduced DO_SS_EXTERNAL flag
        ...
        IF ( DO_UPWELLING ) THEN
          ...
          IF ((DO_SSCORR_NADIR.OR.DO_SSCORR_OUTGOING).OR.DO_SS_EXTERNAL) THEN
            DO Q = 1, N_TOTALCOLUMN_WFS
              DO UT = 1, N_USER_LEVELS
                COLUMNWF(Q,UT,IBEAM,UPIDX) = &
                  COLUMNWF(Q,UT,IBEAM,UPIDX) + COLUMNWF_DB(Q,UT,IBEAM)
              ENDDO
            ENDDO
          ENDIF
        ENDIF
