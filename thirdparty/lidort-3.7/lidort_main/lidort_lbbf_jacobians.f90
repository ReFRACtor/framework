...
!  help variables
      ...
      REAL(fpk) :: H2, H5, SM, TM, SF, COSMUM, MUMP, MUMM, SUM, FAC, FACTOR, omega1_odelt, Del1
...
!  BVProblem, Special Case, N = 1
      ...
      if ( nlayers .eq. 1 ) then
         if ( JB.eq.0 ) then
            ...
            do i = 1, nstreams
               i1 = i + nstreams
               SCOL2_BWF(i)  = - L_T_WUPPER_Gp2(i,n)
               SCOL2_BWF(i1) = - L_T_WLOWER_Gp2(i1,n) + Reflec(i)
            enddo
         endif
      endif
   ...
   IF ( DO_INCLUDE_SURFACE ) THEN
      ...
      IF ( DO_BRDF_SURFACE ) THEN
         DO UM = 1, N_USER_STREAMS
            FACTOR = DOT_PRODUCT(L_IDOWN(1:nstreams),UBRDF_F(M,UM,1:NSTREAMS))
            L_BOA_MSSOURCE(UM) = SURFACE_FACTOR * FACTOR
            L_BOA_MSSOURCE(UM) = L_BOA_MSSOURCE(UM) + USER_EMISSIVITY(UM)
         ENDDO
      ...
      ELSE
         ...
         L_BOA_MSSOURCE(1:N_USER_STREAMS) = FACTOR
         L_BOA_MSSOURCE(1:N_USER_STREAMS) = L_BOA_MSSOURCE(1:N_USER_STREAMS) + EMISS
