...
!  Introduce condition on use of TCUTOFF 5/14/15

        do n = 1, nlayers
         local_do_thermal = ( layermask_up(n).and. deltau_vert(n).gt.TCUTOFF )
         if ( local_do_thermal ) then
!  Regular 
...
!  Linearization of direct solution: whole layer source terms
          IF ( do_atmos_linearization .and.layer_vary_flag(n) ) THEN
...
!  Introduce condition on use of TCUTOFF 5/14/15
        ...
        do n = 1, nlayers
         local_do_thermal = ( layermask_dn(n).and. deltau_vert(n).gt.TCUTOFF )
         if ( local_do_thermal ) then
!  Regular 
...
SUBROUTINE thermal_gfsolution_plus                           &
      ( DO_UPWELLING, DO_DNWELLING, DO_ADDITIONAL_MVOUT,     & !input
        DO_THERMAL_TRANSONLY, DO_ATMOS_LINEARIZATION,        & !input
...
!  Whole layer solutions
     ...
     DO n = 1, nlayers
       IF ( deltau_Power(n,2).gt.TCUTOFF ) then
        DO aa = 1, nstreams
         aa1 = aa + nstreams
         k1 = quad_streams(aa)
...
!  Offgrid: only for quadrature or mean-value output
     ...
     IF ( do_additional_mvout .and. n_partlayers .gt. 0 ) THEN
       DO ut = 1, n_PARTLAYERS
         n  = partlayers_layeridx(ut)
         if ( deltau_Power(n,2).gt.TCUTOFF ) then
!  regular
...
!   -------loop over offgrid optical depths. Invoke layer existence.
...
!    5/14/15.  Solutions will be zero for cutoff layers
...
!  Start layer loop. Invoke existence due to cutoff

        DO n = 1, nlayers
         if ( layermask_up(n) .and. deltau_power(n,2).gt.TCUTOFF ) then
...
!  Start layer loop
        ...
        DO n = 1, nlayers
         if ( layermask_dn(n) .and. deltau_power(n,2).gt.TCUTOFF ) then
