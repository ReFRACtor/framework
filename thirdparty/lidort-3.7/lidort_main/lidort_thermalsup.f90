...
SUBROUTINE THERMAL_SETUP &
      ( DO_UPWELLING, DO_DNWELLING,                                &
        DO_THERMAL_TRANSONLY, DO_MSMODE_THERMAL,                   & ! Add argument 1/13/12
...
!  Thermal Cutoff argument added 5/14/15
...
!  Introduce condition on use of TCUTOFF 5/14/15
        ...
        do n = 1, nlayers
         if ( layermask_up(n).and. deltau_vert(n).gt.TCUTOFF ) then
...
!  Introduce condition on use of TCUTOFF 5/14/15
        ...
        do n = 1, nlayers
         if ( layermask_dn(n).and. deltau_vert(n).gt.TCUTOFF ) then
