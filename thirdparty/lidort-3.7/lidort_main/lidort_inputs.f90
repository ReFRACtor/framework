...
!     R. Spurr, RT SOLUTIONS Inc.
      ...
      REAL(fpk), intent(out) :: THERMAL_CUTOFF ! New 5/14/15
...
!  basic numbers
      ...
      THERMAL_CUTOFF  = ZERO    ! New 5/14/15
      EARTH_RADIUS      = ZERO
...
!     Added, 5/14/15 for use with Thermal Solutions
      ...
      REAL(fpk), intent(inout) :: THERMAL_CUTOFF
...
!  Thermal cutoff (Suugested code for file-read. 5/14/15
!    Default = 1.0d-08

      IF ( DO_THERMAL_EMISSION ) THEN
        THERMAL_CUTOFF = 1.0d-08
!        PAR_STR = 'Thermal Cutoff Optical depth'
!        IF (GFINDPAR ( FILUNIT, PREFIX, ERROR, PAR_STR)) &
!             READ (FILUNIT,*,ERR=998) THERMAL_CUTOFF
!        CALL FINDPAR_ERROR ( ERROR, PAR_STR, STATUS, NM, MESSAGES, ACTIONS )
      ENDIF
