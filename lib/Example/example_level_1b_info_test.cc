...
BOOST_AUTO_TEST_CASE(level1b_list)
{
    ...
    std::vector<std::shared_ptr<Level1b>> level1b_series = ex_lev1_info.level1b_list();
    for (int i = 0; i < level1b_series.size(); i++) {
        BOOST_CHECK_CLOSE(level1b_series[i].get()->latitude(0).value, expected_lats[i], 1e-3);
    }
}

BOOST_AUTO_TEST_CASE(obs_distance)
{
    double expected_dist = 4249.6243199759228;
    ...
    std::shared_ptr<Level1b> example_level_1b_ptr(new ExampleLevel1b(h_file, "2014090915251774"));
    double dist = example_level_1b_info.obs_distance(std::move(example_level_1b_ptr), 0.0, 0.0);
    BOOST_CHECK_CLOSE(dist, expected_dist, 1e-8);
    h_file->close();
}
...
BOOST_AUTO_TEST_CASE(closest_obs_n_distance)
{
    ...
    std::vector<std::shared_ptr<Level1b>> closest_5 = example_level_1b_info.closest_obs_n(0.0, 0.0, 5);
    ...
    for (int i = 0; i < closest_5.size(); i++) {
        // TODO: Add support for spec index when support added to Level1bInfo::obs_distance et al.
        BOOST_CHECK_EQUAL(closest_5[i].get()->time(0), expected_times[i]);
    }
}
...
BOOST_AUTO_TEST_CASE(closest_obs_distance)
{
    ...
    std::shared_ptr<Level1b> closest = example_level_1b_info.closest_obs(0.0, 0.0);

    Time expected_time = Time::time_unix(1410276325.6450486);
    // TODO: Add support for spec index when support added to Level1bInfo::obs_distance et al.
    BOOST_CHECK_EQUAL(closest.get()->time(0), expected_time);
}
...
BOOST_AUTO_TEST_CASE(closest_obs_n_time)
{
    ...
    std::vector<std::shared_ptr<Level1b>> closest_5 = example_level_1b_info.closest_obs_n(Time::parse_time("2014-09-09T15:30:25.645049Z"), 5);
