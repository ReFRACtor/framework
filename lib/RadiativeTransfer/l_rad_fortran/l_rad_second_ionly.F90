module l_rad_second_ionly_m
  ! Specialized version that calculates I correction only.
  !
  ! NOTE: This uses the same function name as l_rad_second_m. This is
  ! actually ok, because we have them in separate modules. When using
  ! this, we can create a local alias, so something like
  !
  ! use l_rad_second_ionly_m, L_rad_second_ionly => L_rad_second
...
! fac or (set to 1.d0 for including shadowing).
      ...
      subroutine L_rad_second &
       (nlay,nicoef,npar,nmug,nphibrdf,surftype, & !I
...
!  inputs
      ...
      integer nlay,nicoef,npar,nmug,nphibrdf
...
!  local variables
      ...
      double precision L_R2c(nlay,npar)
      double precision L_R2cscal(nlay,npar)
      double precision Ls_R2c(nspars)
...
!          ndcoefs = min(2*nmug-1,ncoefs(layer))
          ...
          call L_setZm &
            (m,nmug,npar,ndcoefs,linearize, & !I
...
!  Add Fourier components to the totals
        ...
        call endfou(m,nfoumax,epsilon,xmu,nmug,R2c,nextm)
...
!  local variables
      ...
      double precision L_S1(nlay,npar)
      double precision L_S2(nlay,npar)
      double precision L_S3(nlay,npar)
      double precision L_S4(nlay,npar)
      double precision Ls_S1(nspars),Ls_S2(nspars)
      double precision Ls_S3(nspars),Ls_S4(nspars)
      ...
      double precision V4(4),V6(4)
      double precision L_V1(4,nlay,npar),L_V2
      double precision L_V4(4,nlay,npar),L_V6
      double precision Ls_V1(4,nspars),Ls_V2(4,nspars)
      double precision Ls_V4(4,nspars),Ls_V6(4,nspars)
...
!  Part 1: the vector calculation
        ...
        do l = 1, 2
          ...
          enddo
            ...
            V2(l) = x0*phhl(k)*R1c(1,k,1,l)+ &
                    a*Prc(1,k,1,l)*hl(k)/4.d0
            if (s_linearize) then
              Ls_V2(l,:) = x0*phhl(k)*Ls_R1c(1,k,1,l,:)
              Ls_S1(:) = Ls_S1(:)+a*Ptc(1,k,1,l)* &
                         Ls_V1(l,:)*w(k)
              Ls_S2(:) = Ls_S2(:)+a*Ptc(2,k,l,1)*Ls_V2(l,:)*w(k)
            endif
            if (linearize) then
            do p = 1, npar
              do n = 1, nlay
                L_S1(n,p) = L_S1(n,p)+a*Ptc(1,k,1,l)* &
                            L_V1(l,n,p)*w(k)
                if (n .eq. layer) then
                  L_S1(n,p) = L_S1(n,p)+(L_a(p)*Ptc(1,k,1,l)+ &
                              a*L_Ptc(1,k,1,l,p))*V1(l)*w(k)
                endif
                L_V2 = (L_x0(n,p)*phhl(k)+ &
                       x0*L_phhl(k,n,p))*R1c(1,k,1,l)+ &
                       x0*phhl(k)*L_R1c(1,k,1,l,n,p)+ &
                       a*Prc(1,k,1,l)*L_hl(k,n,p)/4.d0
                if (n .eq. layer) then
                  L_V2 = L_V2+(L_a(p)*Prc(1,k,1,l)+ &
                         a*L_Prc(1,k,1,l,p))*hl(k)/4.d0
                endif
                L_S2(n,p) = L_S2(n,p)+a*Ptc(2,k,l,1)*L_V2*w(k)
                if (n .eq. layer) then
                  L_S2(n,p) = L_S2(n,p)+(L_a(p)*Ptc(2,k,l,1)+ &
                              a*L_Ptc(2,k,l,1,p))*V2(l)*w(k)
                endif
              enddo
            enddo
            endif
        ...
        if (m .gt. 0) then
          do l = 3, 4
            ...
            enddo
              ...
              V6(l) = x0*phhl(k)*R1s(1,k,1,l)+ &
                        a*Prs(1,k,1,l)*hl(k)/4.d0
              if (s_linearize) then
                Ls_V6(l,:) = x0*phhl(k)*Ls_R1s(1,k,1,l,:)
                Ls_S3(:) = Ls_S3(:)+a*Pts(1,k,1,l)*Ls_V4(l,:)*w(k)
                Ls_S4(:) = Ls_S4(:)+a*Pts(2,k,l,1)*Ls_V6(l,:)*w(k)
              endif
              if (linearize) then
              do p = 1, npar
                do n = 1, nlay
                  L_S3(n,p) = L_S3(n,p)+ &
                              a*Pts(1,k,1,l)*L_V4(l,n,p)*w(k)
                  if (n .eq. layer) then
                    L_S3(n,p) = L_S3(n,p)+(L_a(p)*Pts(1,k,1,l)+ &
                                L_Pts(1,k,1,l,p))*V4(l)*w(k)
                  endif
                  L_V6 = (L_x0(n,p)*phhl(k)+ &
                         x0*L_phhl(k,n,p))*R1s(1,k,1,l)+ &
                         x0*phhl(k)*L_R1s(1,k,1,l,n,p)+ &
                         a*Prs(1,k,1,l)*L_hl(k,n,p)/4.d0
                  if (n .eq. layer) then
                    L_V6 = L_V6+(L_a(p)*Prs(1,k,1,l)+ &
                           a*L_Prs(1,k,1,l,p))*hl(k)/4.d0
                  endif
                  L_S4(n,p) = L_S4(n,p)+a*Pts(2,k,l,1)*L_V6*w(k)
                  if (n .eq. layer) then
                    L_S4(n,p) = L_S4(n,p)+(L_a(p)*Pts(2,k,l,1)+ &
                                  a*L_Pts(2,k,l,1,p))*V6(l)*w(k)
                  endif
                enddo
              enddo
              endif
...
!  Part 1: vector
        ...
        do l = 1, 2
          S1 = S1+a*Ptc(1,k,1,l)*V1(l)*w(k)
!write(0,*) 'S1++',S1,a,Ptc(1,k,1,l),V1(l),w(k)
          S2 = S2+V2(l)*a*Ptc(2,k,l,1)*w(k)
        enddo
        if (m .gt. 0) then
          do l = 3,4
            S3 = S3+a*Pts(1,k,1,l)*V4(l)*w(k)
            S4 = S4+V6(l)*a*Pts(2,k,l,1)*w(k)
          enddo
        endif
...
!  recursion on the linearised quantities
      ...
      do p = 1, npar
        do n = 1, nlay
          L_R2c(n,p) = L_R2c(n,p)*facl+R2c*L_facl(n,p)
          L_R2cscal(n,p) = L_R2cscal(n,p)*facl+ &
                           R2cscal*L_facl(n,p)
          L_R2c(n,p) = L_R2c(n,p)+L_S1(n,p)/2.d0+ &
                       L_S2(n,p)/2.d0
          L_R2cscal(n,p) = L_R2cscal(n,p)+L_S1scal(n,p)/2.d0+ &
                           L_S2scal(n,p)/2.d0
          if (m .gt. 0) then
            L_R2c(n,p) = L_R2c(n,p)+L_S3(n,p)/2.d0- &
                         L_S4(n,p)/2.d0
          endif
        enddo
      enddo
...
!  recursion on the linearised surface quantities
      ...
      if (s_linearize) then
        ...
        Ls_R2c(:) = Ls_R2c(:)+Ls_S1(:)/2.d0+Ls_S2(:)/2.d0
        Ls_R2cscal(:) = Ls_R2cscal(:)+Ls_S1scal(:)/2.d0+ &
                        Ls_S2scal(:)/2.d0
        if (m .gt. 0) then
          Ls_R2c(:) = Ls_R2c(:)+Ls_S3(:)/2.d0-Ls_S4(:)/2.d0
        endif
      endif
...
!  recursion on the originals
      ...
      R2c = R2c+S1/2.d0+S2/2.d0
      R2cscal = R2cscal+S1scal/2.d0+S2scal/2.d0
      if (m .gt. 0) then
        R2c = R2c+S3/2.d0-S4/2.d0
      endif
...
!  inputs
      ...
      integer m,nlay,npar,nspars
      logical linearize,s_linearize
      double precision phi,R2c,R2cscal
      double precision L_R2c(nlay,npar)
      double precision L_R2cscal(nlay,npar)
      double precision Ls_R2c(nspars)
      double precision Ls_R2cscal(nspars)
...
!  scalar correction
      ...
      Icorr = Icorr+fac*(R2c-R2cscal)*cosmph
      if (s_linearize) then
        Ls_Icorr(:) = Ls_Icorr(:)+fac*(Ls_R2c(:)-Ls_R2cscal(:))*cosmph
      endif
      if (linearize) then
      do n = 1, nlay
        do p = 1, npar
          L_Icorr(n,p) = L_Icorr(n,p)+fac*(L_R2c(n,p)- &
...
!  inputs
      ...
      integer m,nfoumax,nmug
      logical nextm
      double precision epsilon,xmu(nmug+2),R2c
...
!  local variables
      ...
      nextm = .false.
      count = 0
      if ((xmu(nmug+1)*dabs(R2c)) .gt. epsilon) count = count+1
      if (count .eq. 1) nextm = .true.
...
END module l_rad_second_ionly_m
