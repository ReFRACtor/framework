...
BOOST_AUTO_TEST_CASE(basic)
{
    ...
    AbsorberVmrMet avmr(met_data, pressure_sigma, 1.0, true, "H2O");
    ...
    for(int i = 0; i < grid_expect.rows(); ++i)
        BOOST_CHECK_CLOSE(avmr.volume_mixing_ratio(pressure_sigma->pressure_grid()(i).value).value(), grid_expect(i), 1e-3);
}
