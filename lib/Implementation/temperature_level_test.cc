...
public:
  TemperatureLevelTest(const boost::shared_ptr<Pressure>& press_in)
    : pressure(press_in)
  {
    test_temp.resize(19);
    ...
    if(pressure->type_preference() == Pressure::PREFER_INCREASING_PRESSURE)
      temp_lev.reset(new TemperatureLevel(test_temp, pressure));
    else
      temp_lev.reset(new TemperatureLevel(test_temp.reverse(blitz::firstDim), pressure));
  }
...
BOOST_AUTO_TEST_CASE(basic)
{
  ...
  sv.update_state(temp_test.test_temp);
  std::cerr << sv << "\n";
  temp_test.check_results(temp_test.temp_lev);
}
...
BOOST_AUTO_TEST_CASE(reversed)
{
  ...
  TemperatureLevelTest temp_test(pressure_reverse);

  StateVector sv;
  sv.add_observer(*temp_test.temp_lev);
  
  temp_test.check_results(temp_test.temp_lev);
  sv.update_state(temp_test.test_temp.reverse(blitz::firstDim));
  std::cerr << sv << "\n";
  temp_test.check_results(temp_test.temp_lev);
}
