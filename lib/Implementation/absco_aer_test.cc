...
BOOST_AUTO_TEST_CASE(basic)
{
  ...
  if(false) {
    std::cerr << setprecision(20) << std::scientific
	      ...
	      << f.temperature_grid()(53, Range(0,7)) << "\n"
              ...
              << f.read<double>(5000.0)(Range(0,7), 53, 0) << "\n";
  }
  ...
  Array<double, 1> readsub(f.read<double>(5000)(Range(0,7), 53, 0));
  ...
  abs_expect = 2.4592514036610671e-23, 2.2321262075073958e-23,
    1.4605750875638199e-23;
  for(int i = 0; i < 3; ++i)
    BOOST_CHECK_CLOSE(f.absorption_cross_section
		      (5000, pv(i), tv(i), bv(i)).value,
		      abs_expect(i), 1e-4);
  for(int i = 0; i < 3; ++i)
  ...
  AutoDerivative<double> absv = f.absorption_cross_section(5000, pvd, tvd, 
							   bvd).value;
  AutoDerivative<double> absvscale = 
    fscale.absorption_cross_section(5000, pvd, tvd, 
				    bvd).value;
  BOOST_CHECK_CLOSE(absv.value(), abs_expect(1), 1e-3);
  BOOST_CHECK_CLOSE(absvscale.value(), abs_expect(1) * table_scale, 1e-3);
  double epsilon = 1e-3;
  tvd.value += epsilon;
  double dabs_dt = (f.absorption_cross_section(5000, pvd, tvd, 
					       bvd).value.value() - 
		    absv.value()) / epsilon;
  tvd.value -= epsilon;
  bvd.value += epsilon;
  double dabs_db = (f.absorption_cross_section(5000, pvd, tvd, 
					       bvd).value.value() - 
		    absv.value()) / epsilon;
...
BOOST_AUTO_TEST_CASE(full_wn_range)
{
  ...
  AbscoAer a(absco_aer_data_dir() + "/CO2_04760-06300_v0.0_init.chunk.nc");
