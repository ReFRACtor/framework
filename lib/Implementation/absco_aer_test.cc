...
BOOST_AUTO_TEST_CASE(basic)
{
  ...
  AbscoAer f(absco_aer_data_dir() + "/CO2_00600-01200_v0.0_init.nc");
  // Note scale here is a nonsense value
  double table_scale = 1.2;
  AbscoAer fscale(absco_aer_data_dir() + "/CO2_00600-01200_v0.0_init.nc",
		  table_scale);
  ...
  if (true) {
    std::cerr << setprecision(20) << std::scientific
	      << "# This is the expected pressure grid.\n"
              << f.pressure_grid() << "\n"
	      << "# This is tsub_expect\n"
	      << f.temperature_grid()(53, Range::all()) << "\n"
              << "# This is readsub_expect\n";
    //              << f.read<double>(12929.94)(53, Range::all(), 0) << "\n";
  }
