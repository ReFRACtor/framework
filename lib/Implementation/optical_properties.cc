...
: rayleigh_optical_depth_(rayleigh_od),
  ...
  aerosol_optical_depth_per_particle_(aerosol_od), 
  aerosol_single_scattering_albedo_per_particle_(aerosol_ssa)
{
                                     ...
                                     const boost::shared_ptr<Aerosol>& aerosol)
{
    ...
    aerosol_optical_depth_per_particle_ = aerosol->optical_depth_each_layer(wn);

    aerosol_single_scattering_albedo_per_particle_.resize(aerosol_optical_depth_per_particle_.shape(), aerosol_optical_depth_per_particle_.number_variable());
    for(int particle_idx = 0; particle_idx < aerosol_optical_depth_per_particle_.cols(); ++particle_idx) {
      aerosol_single_scattering_albedo_per_particle_(Range::all(), particle_idx) = aerosol->ssa_each_layer(wn, particle_idx, aerosol_optical_depth_per_particle_(Range::all(), particle_idx));
    }
...
ArrayAd<double, 1> OpticalProperties::aerosol_optical_depth_per_layer() const
{
    // aerosol component
    ArrayAd<double, 2> aer_od_per_p(aerosol_optical_depth_per_particle());
    if (aerosol_optical_depth_per_layer_.rows() == 0 and aer_od_per_p.cols() > 0) {
        firstIndex i1; secondIndex i2;
        Range ra = Range::all();

        aerosol_optical_depth_per_layer_.resize(aer_od_per_p.rows(), aer_od_per_p.number_variable());

        aerosol_optical_depth_per_layer_.value() = sum(aer_od_per_p.value(), i2);
        if (!aer_od_per_p.is_constant()) {
            for(int lay_idx = 0; lay_idx < aer_od_per_p.value().rows(); lay_idx++) {
                aerosol_optical_depth_per_layer_.jacobian()(lay_idx, ra) = sum(aer_od_per_p.jacobian()(lay_idx, ra, ra)(i2, i1), i2);
            }
        }
    }

    return aerosol_optical_depth_per_layer_;
}
...
ArrayAd<double, 1> OpticalProperties::total_optical_depth() const
{
    if (total_optical_depth_.rows() == 0) {
        ...
        ArrayAd<double, 1> aerosol_od(aerosol_optical_depth_per_layer());
...
ArrayAd<double, 2> OpticalProperties::total_single_scattering_albedo() const
{
}
