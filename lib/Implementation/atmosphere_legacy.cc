...
REGISTER_LUA_DERIVED_CLASS(AtmosphereLegacy, RtAtmosphere)
...
.def(luabind::constructor<const boost::shared_ptr<Absorber>&,
	     ...
	     const std::vector<boost::shared_ptr<Altitude> >&,
	     const boost::shared_ptr<Constant>&>())
.def("pressure", &AtmosphereLegacy::pressure_ptr)
.def("temperature", &AtmosphereLegacy::temperature_ptr)
.def("ground", &AtmosphereLegacy::ground)
REGISTER_LUA_END()
#endif
...
AtmosphereLegacy::AtmosphereLegacy(const boost::shared_ptr<Absorber>& absorberv,
...
void AtmosphereLegacy::initialize()
{
  if(!absorber)
    throw Exception("Absorber is not allowed to be null in AtmosphereLegacy");
  if(!pressure)
    throw Exception("Pressure is not allowed to be null in AtmosphereLegacy");
  if(!temperature)
    throw Exception("Temperature is not allowed to be null in AtmosphereLegacy");
...
void AtmosphereLegacy::set_aerosol(boost::shared_ptr<Aerosol>& new_aerosol, StateVector& Sv) {
...
ArrayAd<double, 3> 
AtmosphereLegacy::scattering_moment_common(double wn, int nummom, int numscat) const
{
...
void AtmosphereLegacy::reset_timer()
{ 
...
std::string AtmosphereLegacy::timer_info() const
{
...
void AtmosphereLegacy::notify_update(const Aerosol& UNUSED(A))
{
  wn_tau_cache = -1;
...
bool AtmosphereLegacy::fill_cache(double wn, int spec_index) const
{
...
void AtmosphereLegacy::calc_intermediate_variable(double wn, int spec_index) 
const
{
...
void AtmosphereLegacy::calc_rt_parameters
...
ArrayAdWithUnit<double, 1> AtmosphereLegacy::altitude(int spec_index) const
{
  range_check(spec_index, 0, number_spectrometer());
...
ArrayAd<double, 1> AtmosphereLegacy::atmosphere_blackbody(double wn, int UNUSED(spec_index)) const
{
...
AutoDerivative<double> AtmosphereLegacy::surface_blackbody(double wn, int spec_index) const
{
    if (!surface_temp) {
        Exception error;
...
boost::shared_ptr<AtmosphereLegacy> AtmosphereLegacy::clone() const
{
  ...
  boost::shared_ptr<AtmosphereLegacy> res
    (new AtmosphereLegacy(absorber_clone, pressure_clone, temperature_clone, rayleigh_clone,
                          aerosol_clone, rh_clone, ground_clone, alt_clone, 
                          constant));
  return res;
}

void AtmosphereLegacy::print(std::ostream& Os) const
{
  Os << "AtmosphereLegacy:\n";
  OstreamPad opad(Os, "    ");
  Os << "  Constant:\n";
...
void AtmosphereLegacy::attach_children_to_sv(StateVector& statev)
{
