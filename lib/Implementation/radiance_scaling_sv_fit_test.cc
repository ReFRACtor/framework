...
BOOST_FIXTURE_TEST_SUITE(radiance_scaling_sv_fit, LambertianConfigurationFixture)
...
BOOST_AUTO_TEST_CASE(serialization)
{
  ...
  DoubleWithUnit band_ref(0.77, "cm^-1");
  std::string band_name = "ABand";
  boost::shared_ptr<RadianceScalingSvFit> rs = boost::make_shared<RadianceScalingSvFit>(coeff, band_ref, band_name);
  std::string d = serialize_write_string(rs);
  if(false)
    std::cerr << d;
  boost::shared_ptr<RadianceScalingSvFit> rsr = serialize_read_string<RadianceScalingSvFit>(d);
  SpectralDomain pixel_grid = config_instrument->pixel_spectral_domain(0);
  std::vector<int> pixel_list;
  for(int i = 0; i < pixel_grid.data().rows(); i++)
    pixel_list.push_back(i);
