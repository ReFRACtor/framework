...
BOOST_AUTO_TEST_CASE(basic)
{
  ...
  BOOST_CHECK(max(abs(rad_diff)) < 1.5e5);
...
BOOST_AUTO_TEST_CASE(serialization)
{
  ...
  for(int i = 0; i < pixel_grid.data().rows(); i++)
    pixel_list.push_back(i);
  boost::shared_ptr<RadianceScalingLinearFit> rs = boost::make_shared<RadianceScalingLinearFit>(spec_meas, band_ref, "A-Band", true);
  std::string d = serialize_write_string(rs);
  if(false)
    std::cerr << d;
  boost::shared_ptr<RadianceScalingLinearFit> rsr = serialize_read_string<RadianceScalingLinearFit>(d);
  ...
  rsr->apply_correction(pixel_grid, pixel_list, meas_rad_test);

  Array<double, 1> scaling = rsr->radiance_scaling_coeff();
  double offset = rsr->radiance_offset();
