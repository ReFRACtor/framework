...
{
    boost::shared_ptr<HdfFile> hfile(new HdfFile(test_data_dir() + "in/common/l1b_example_data.h5"));
    ...
    BOOST_CHECK_CLOSE(l1b_cache.sounding_zenith(0).value, 34.251796722412109, 1e-4);
    BOOST_CHECK_CLOSE(l1b_cache.sounding_zenith(1).value, 34.244930267333984, 1e-4);
    BOOST_CHECK_CLOSE(l1b_cache.sounding_zenith(2).value, 34.242877960205078, 1e-4);

    BOOST_CHECK_CLOSE(l1b_cache.sounding_azimuth(0).value, 138.78759765625, 1e-4);
    BOOST_CHECK_CLOSE(l1b_cache.sounding_azimuth(1).value, 138.78660583496094, 1e-4);
    BOOST_CHECK_CLOSE(l1b_cache.sounding_azimuth(2).value, 138.79510498046875, 1e-4);

    BOOST_CHECK_CLOSE(l1b_cache.stokes_coefficient(0)(0), 0.5, 1e-8);
    BOOST_CHECK_CLOSE(l1b_cache.stokes_coefficient(0)(1), 0.49999275803565979, 1e-8);
    BOOST_CHECK_CLOSE(l1b_cache.stokes_coefficient(0)(2), -0.0026892200112342834, 1e-6);
    BOOST_CHECK_CLOSE(l1b_cache.stokes_coefficient(0)(3), 0.0, 1e-8);

    BOOST_CHECK_CLOSE(l1b_cache.stokes_coefficient(1)(0), 0.5, 1e-8);
    BOOST_CHECK_CLOSE(l1b_cache.stokes_coefficient(1)(1), 0.4999927282333374, 1e-8);
    BOOST_CHECK_CLOSE(l1b_cache.stokes_coefficient(1)(2), -0.0026979264803230762, 1e-6);
    BOOST_CHECK_CLOSE(l1b_cache.stokes_coefficient(1)(3), 0.0, 1e-8);

    BOOST_CHECK_CLOSE(l1b_cache.stokes_coefficient(2)(0), 0.5, 1e-8);
    BOOST_CHECK_CLOSE(l1b_cache.stokes_coefficient(2)(1), 0.49999341368675232, 1e-8);
    BOOST_CHECK_CLOSE(l1b_cache.stokes_coefficient(2)(2), -0.0025685979053378105, 1e-6);
    BOOST_CHECK_CLOSE(l1b_cache.stokes_coefficient(2)(3), 0.0, 1e-8);

    BOOST_CHECK_CLOSE(l1b_cache.relative_velocity(0).value, 3350.394775390625, 1e-4);
    ...
    BOOST_CHECK_CLOSE(l1b_cache.radiance(0).data()(1005), 6.0255898020264018e+19, 1e-4);
    BOOST_CHECK_EQUAL(l1b_cache.radiance(1).data().extent(blitz::firstDim), 1016);
    BOOST_CHECK_CLOSE(l1b_cache.radiance(1).data()(1005), 2.3494947203480289e+19, 1e-4);
