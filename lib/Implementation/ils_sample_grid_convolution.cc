...
REGISTER_LUA_DERIVED_CLASS(IlsSampleGridConvolution, Ils)
...
blitz::Array<double, 1> IlsSampleGridConvolution::apply_ils(const blitz::Array<double, 1>& Hres_wn,
                                                            ...
                                                            const std::vector<int>& Pixel_list) const
{
  ...
  Array<double, 1> disp_wn(sample_grid_->sample_grid().data());
...
double IlsSampleGridConvolution::integrate(const blitz::Array<double, 1>& x,
...
AutoDerivative<double> IlsSampleGridConvolution::integrate(const blitz::Array<double, 1>& x,
...
// See base class for description.
ArrayAd<double, 1> IlsSampleGridConvolution::apply_ils(const blitz::Array<double, 1>& Hres_wn,
                                                       ...
                                                       const std::vector<int>& Pixel_list) const
{
  ...
  if(Hres_wn.rows() != Hres_rad.rows())
    throw Exception("wave_number and radiance need to be the same size");
  ArrayAd<double, 1> disp_wn(sample_grid_->sample_grid().data_ad());
...
boost::shared_ptr<Ils> IlsSampleGridConvolution::clone() const
{
  return boost::shared_ptr<Ils>(new IlsSampleGridConvolution(sample_grid_->clone(),
			   ils_func, ils_half_width_));
}

void IlsSampleGridConvolution::print(std::ostream& Os) const
{ 
  Os << "IlsSampleGridConvolution:\n";
  OstreamPad opad(Os, "  ");
  opad << *sample_grid_ << "\n" << *ils_func << "\n"
