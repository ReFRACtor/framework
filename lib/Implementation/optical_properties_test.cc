...
BOOST_AUTO_TEST_CASE(sv_basis_jacobian)
{
    ...
    ArrayAd<double, 3> tot_pf_expt_1 = atm_legacy->phase_function_moments_wrt_state_vector(test_wn, test_chan);
    ...
    ArrayAd<double, 3> tot_pf_expt_2 = atm_legacy->phase_function_moments_wrt_state_vector(test_wn, test_chan, 200, 1);
...
BOOST_AUTO_TEST_CASE(rt_basis_jacobian)
{
    ...
    BOOST_CHECK_MATRIX_CLOSE_TOL(atm_legacy->optical_depth_wrt_rt(test_wn, test_chan).value(), opt_prop_wrt_rt.total_optical_depth().value(), 1e-10);
    BOOST_CHECK_MATRIX_CLOSE_TOL(atm_legacy->optical_depth_wrt_rt(test_wn, test_chan).jacobian(), opt_prop_wrt_rt.total_optical_depth().jacobian(), 1e-10);

    BOOST_CHECK_MATRIX_CLOSE_TOL(atm_legacy->single_scattering_albedo_wrt_rt(test_wn, test_chan).value(), opt_prop_wrt_rt.total_single_scattering_albedo().value(), 1e-10);
    BOOST_CHECK_MATRIX_CLOSE_TOL(atm_legacy->single_scattering_albedo_wrt_rt(test_wn, test_chan).jacobian(), opt_prop_wrt_rt.total_single_scattering_albedo().jacobian(), 1e-10);
    ...
    // Total pf which include rayleigh portion which is not exposed by Atmosphere interface
    ArrayAd<double, 3> tot_pf_expt_1 = atm_legacy->phase_function_moments_wrt_rt(test_wn, test_chan);
    ...
    // Check that phase function can be recomputed with a different number of moments and scattering
    ArrayAd<double, 3> tot_pf_expt_2 = atm_legacy->phase_function_moments_wrt_rt(test_wn, test_chan, 200, 1);
